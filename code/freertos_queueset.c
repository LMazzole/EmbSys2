void vAFunction (void)
{
	...
	xSemaphoreTake (xSemaphore, 0);         /*Take the semaphore, so it starts empty. A block time of zero can be used as the semaphore is guaranteed to be available - it has just been created.*/
	xQueueSet = xQueueCreateSet (10);       /*Create a queue set large enough to hold an event for every space in everyqueue and semaphore that is to be added to the set. */
	xQueueAddToSet (xQueue1, xQueueSet);    /*Add the queues and semaphores to the set.*/
	xQueueAddToSet (xQueue2, xQueueSet);
	xQueueAddToSet (xSemaphore, xQueueSet);
	for (;;)
	{
		xActivatedMember = xQueueSelectFromSet (xQueueSet, pdMS_TO_TICKS (200)); /*Block to wait for something available from the queues or semaphore added to the set. Don't block longer than 200ms.*/
		...
		{
			SemaphoreTake (xActivatedMember, 0);
			/*Process Semaphore*/
			break;
		}
		else
